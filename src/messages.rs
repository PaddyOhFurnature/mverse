...
impl Action {
    ...
    pub fn is_terrain(&self) -> bool {
        matches!(
            self,
            Self::SetVoxel { .. } | Self::RemoveVoxel { .. } | Self::FillRegion { .. }
        )
    }
}
...
mod tests {
    ...
    fn test_voxel_operation_signature() {
        ...
        let mut op =
            VoxelOperation::new(coord, Material::Stone, identity.peer_id().clone(), 100, vc);
    ...
    fn test_voxel_operation_crdt_ordering() {
        ...
        assert!(
            winner.author.to_bytes()
                > if winner.author == op3.author {
                    op4.author
                } else {
                    op3.author
                }
    ...
    fn test_signed_operation_affecting_chunks() {
        ...
        assert!(
            !chunks.is_empty(),
            "SetVoxel must affect at least one chunk"
        );
    }
