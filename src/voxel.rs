...
/// Octree root span must be a power of two so recursive halving preserves
/// one-voxel leaves without aliasing odd-sized upper children.
const OCTREE_ROOT_SIZE: i64 = 16_777_216; // next power of two above 12_800_000
...
impl VoxelCoord {
    ...
    pub fn is_valid(&self) -> bool {
        self.x >= 0
            && self.x < VOXEL_GRID_SIZE
            && self.y >= 0
            && self.y < VOXEL_GRID_SIZE
            && self.z >= 0
            && self.z < VOXEL_GRID_SIZE
    }
}
...
impl Octree {
    ...
    pub fn new() -> Self {
        Self {
            ...
            max_depth: 24, // 16,777,216 / 2^24 = 1m leaves
        }
    }
    ...
    pub fn get_voxel(&self, pos: VoxelCoord) -> MaterialId {
        ...
        self.get_recursive(&self.root, pos, 0, 0, 0, OCTREE_ROOT_SIZE)
    }
    ...
    fn set_recursive(
        ...
        if let OctreeNode::Branch { children } = node {
            ...
            let all_same = children
                .iter()
                .all(|child| child.material() == Some(material));
    ...
    for _ in 0..max_steps {
        ...
        if material != MaterialId::AIR {
            ...
            let distance = voxel_x.abs_diff(start_voxel.x).max(
                voxel_y
                    .abs_diff(start_voxel.y)
                    .max(voxel_z.abs_diff(start_voxel.z)),
            ) as i64;
...
mod tests {
    ...
    fn test_octree_adjacent_voxels() {
        ...
        assert_eq!(
            octree.get_voxel(VoxelCoord::new(5001, 5000, 5000)),
            MaterialId::AIR
        );
        assert_eq!(
            octree.get_voxel(VoxelCoord::new(4999, 5000, 5000)),
            MaterialId::AIR
        );
        assert_eq!(
            octree.get_voxel(VoxelCoord::new(5000, 5001, 5000)),
            MaterialId::AIR
        );
        assert_eq!(
            octree.get_voxel(VoxelCoord::new(5000, 5000, 5001)),
            MaterialId::AIR
        );
    }

    #[test]
    fn test_octree_adjacent_voxels_near_odd_split_boundary() {
        let mut octree = Octree::new();

        let pos = VoxelCoord::new(1_312_850, 9_029_745, 3_600_945);
        let adjacent = VoxelCoord::new(1_312_851, 9_029_745, 3_600_945);

        octree.set_voxel(pos, MaterialId::STONE);

        assert_eq!(octree.get_voxel(pos), MaterialId::STONE);
        assert_eq!(octree.get_voxel(adjacent), MaterialId::AIR);
    }
    ...
    fn test_raycast_hit_single_block() {
        ...
        assert!(
            hit.voxel.x >= 95 && hit.voxel.x <= 105,
            "Hit voxel x should be near 100, got {}",
            hit.voxel.x
        );
    ...
    fn test_raycast_max_distance() {
        ...
        assert!(
            hit.is_some(),
            "Should reach distant block with longer range"
        );
    }
    ...
    fn test_raycast_face_normals() {
        ...
        for (origin_voxel, dir, expected_normal) in test_cases {
            ...
            assert_eq!(
                hit.unwrap().face_normal,
                expected_normal,
                "Wrong normal for direction {:?}",
                dir
            );
        }
    }
