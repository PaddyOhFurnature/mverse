...
/// 3D vertex with position, normal, and material color
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct Vertex {
    pub position: Vec3,
    pub normal: Vec3,
    /// Linear-space RGB material color [0..1]
    pub color: Vec3,
}
...
impl Vertex {
    pub fn new(position: Vec3, normal: Vec3) -> Self {
        Self {
            position,
            normal,
            color: Vec3::new(0.3, 0.5, 0.2),
        }
    }

    pub fn with_color(position: Vec3, normal: Vec3, color: Vec3) -> Self {
        Self {
            position,
            normal,
            color,
...
mod tests {
    ...
    fn test_add_vertex() {
        ...
        let v = Vertex::new(Vec3::new(1.0, 2.0, 3.0), Vec3::new(0.0, 1.0, 0.0));
