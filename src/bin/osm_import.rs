...
fn main() {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--pbf"   => { if let Some(v) = args.next() { pbf_path = PathBuf::from(v); } }
            "--cache" => { if let Some(v) = args.next() { cache_dir = PathBuf::from(v); } }
    ...
    let pbf_mb = std::fs::metadata(&pbf_path).map(|m| m.len() / 1_048_576).unwrap_or(0);
    ...
    match metaverse_core::osm::import_pbf_with_log(&pbf_path, &cache_dir, Arc::clone(&log)) {
        Ok(tiles_written) => {
            // Drain any remaining messages.
            std::thread::sleep(std::time::Duration::from_millis(600));
            let remaining: Vec<String> = std::mem::take(&mut *log.lock().unwrap());
            for m in remaining { println!("{m}"); }

            let elapsed = start.elapsed();
            println!();
            println!("✅  Done — {} tiles written in {:.1}s", tiles_written, elapsed.as_secs_f32());
...
fn wipe_flat_bin_files(dir: &std::path::Path) {
    ...
    if removed > 0 {
        println!("🧹  Removed {removed} stale flat .bin files from {}", dir.display());
    }
}
