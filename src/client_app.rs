...
use crate as metaverse_core;
use egui_wgpu::ScreenDescriptor;
use glam::{Mat4, Vec3};
use metaverse_core::{
    billboard::{BillboardPipeline, ModuleBillboards, TerminalScreen},
    chunk::{CHUNK_SIZE_X, CHUNK_SIZE_Y, CHUNK_SIZE_Z, ChunkId},
    ...
    multiplayer::{MultiplayerRuntimeConfig, MultiplayerSystem},
...
fn octree_chunk_has_non_air(octree: &metaverse_core::voxel::Octree, chunk_id: ChunkId) -> bool {
...
fn try_preload_capture_chunk(
    ...
    if let Some(data) = stored {
        if let Some((octree, surface_cache)) = metaverse_core::chunk_loader::decode_stored_chunk(&data)
        {
            if octree_chunk_has_non_air(&octree, chunk_id) {
                chunk_streamer.preload_chunk(chunk_id, octree, surface_cache, None);
...
fn target_alt_m_or_staging(
    alt_m: Option<f64>,
    staging_altitude_m: f64,
    origin_gps: GPS,
) -> f64 {
    alt_m.unwrap_or(origin_gps.alt + staging_altitude_m)
}

fn target_column_voxel_for_gps(lat: f64, lon: f64, reference_alt_m: f64) -> VoxelCoord {
    // Preserve the horizontal column under the active world origin.
    // Using alt=0 skews ECEF x/z for high-elevation inland sites and can shift
    // the target hundreds of meters away from the baked chunks.
    let target_ecef = GPS::new(lat, lon, reference_alt_m).to_ecef();
    VoxelCoord::from_ecef(&target_ecef)
}
...
struct CapturePointConfig {
    ...
    alt_m: Option<f64>,
    #[serde(default)]
    target_voxel_x: Option<i64>,
    #[serde(default)]
    target_voxel_y: Option<i64>,
    #[serde(default)]
    target_voxel_z: Option<i64>,
    #[serde(default)]
    yaw_deg: f32,
...
struct TeleportPointsConfig {
    ...
    #[serde(default = "default_capture_loading_timeout_secs")]
    loading_timeout_secs: u64,
    points: Vec<TeleportPointConfig>,
}
...
struct CaptureShotRecord {
    ...
    point_name: String,
    view: String,
    target_gps: Option<[f64; 2]>,
    player_gps: [f64; 3],
    player_orthometric_alt_m: f64,
    anchor_offset_m: Option<f64>,
    player_local: [f32; 3],
    camera_bearing_deg: Option<f32>,
    ...
    above_material: String,
    below_material: String,
    ...
    loaded_this_frame: Option<usize>,
    game_mode: String,
    player_mode: String,
    observability_mode: String,
    layer_view_mode: String,
    world_dir: String,
    shared_data_root: String,
    screenshot_root: String,
    region: Option<String>,
    active_chunk_id: Option<[i64; 3]>,
    active_chunk_last_modified: Option<u64>,
    active_chunk_non_air_columns: Option<usize>,
    active_chunk_water_columns: Option<usize>,
    active_chunk_road_columns: Option<usize>,
    active_chunk_building_columns: Option<usize>,
    active_chunk_min_ground_y: Option<i64>,
    active_chunk_max_ground_y: Option<i64>,
}
...
#[derive(Debug, Clone)]
enum FrameCaptureKind {
    Manual,
    Route {
        point_name: String,
        target_gps: [f64; 2],
        view: CaptureViewMode,
    },
}
...
impl CaptureRunner {
    ...
    fn current_target_alt_m(&self, origin_gps: GPS) -> Option<f64> {
        self.current_point().map(|point| {
            target_alt_m_or_staging(point.alt_m, self.route.staging_altitude_m, origin_gps)
        })
    }

    fn current_target_column_voxel(&self, origin_gps: GPS) -> Option<VoxelCoord> {
        self.current_point().map(|point| {
            match (point.target_voxel_x, point.target_voxel_y, point.target_voxel_z) {
                (Some(x), Some(y), Some(z)) => VoxelCoord::new(x, y, z),
                _ => target_column_voxel_for_gps(point.lat, point.lon, origin_gps.alt),
            }
        })
    }
    ...
    fn build_capture_request(&self) -> Option<FrameCaptureRequest> {
...
struct TeleportController {
    config: TeleportPointsConfig,
    source_path: String,
    stage: TeleportStage,
...
impl TeleportController {
    fn from_file(path: &Path) -> Result<Self, String> {
        let raw = std::fs::read_to_string(path)
            .map_err(|e| format!("failed to read teleport points {}: {}", path.display(), e))?;
        let mut config: TeleportPointsConfig = serde_json::from_str(&raw)
            .map_err(|e| format!("failed to parse teleport points {}: {}", path.display(), e))?;
        if config.points.is_empty() {
            return Err(format!("teleport points file {} has no points", path.display()));
        }
        for point in &mut config.points {
            if point.label.is_empty() {
                point.label = point.id.clone();
            }
        }
        Ok(Self {
            config,
    ...
    fn current_point(&self) -> Option<&TeleportPointConfig> {
        self.active_index.and_then(|idx| self.config.points.get(idx))
    }

    fn current_index_one_based(&self) -> Option<usize> {
        self.active_index.map(|idx| idx + 1)
    }

    fn current_ground_offset_m(&self) -> f32 {
        self.current_point()
            .and_then(|point| point.ground_offset_m)
            .unwrap_or(self.config.ground_offset_m)
    }

    fn current_target_alt_m(&self, origin_gps: GPS) -> Option<f64> {
        self.current_point().map(|point| {
            target_alt_m_or_staging(point.alt_m, self.config.staging_altitude_m, origin_gps)
        })
    }

    fn current_target_column_voxel(&self, origin_gps: GPS) -> Option<VoxelCoord> {
        self.current_point()
            .map(|point| target_column_voxel_for_gps(point.lat, point.lon, origin_gps.alt))
    }

    fn queue_index(&mut self, one_based_index: usize) -> Result<(), String> {
        if one_based_index == 0 || one_based_index > self.config.points.len() {
            return Err(format!(
                "tp index {} out of range (1..={})",
                one_based_index,
                self.config.points.len()
            ));
        }
        ...
        self.stage = TeleportStage::TeleportPoint;
        self.load_timed_out = false;
        self.load_timeout_started_at = None;
    ...
    fn queue_next(&mut self) -> Result<(), String> {
        let next = match self.active_index {
            Some(idx) => (idx + 1) % self.config.points.len(),
            None => 0,
        };
        self.queue_index(next + 1)
    }

    fn queue_prev(&mut self) -> Result<(), String> {
        let prev = match self.active_index {
            Some(0) | None => self.config.points.len() - 1,
            Some(idx) => idx - 1,
        };
        self.queue_index(prev + 1)
    }

    fn print_help(&self) {
        println!("🧭 Shell teleport commands:");
        println!("   tp <n>     — queue numbered teleport point (1..={})", self.point_count());
        println!("   tp next    — queue the next point");
        println!("   tp prev    — queue the previous point");
        println!("   tp list    — print all numbered points");
        println!("   tp status  — show the current teleport target");
        println!("   tp help    — show this help");
    }

    fn print_list(&self) {
        println!(
            "🧭 Teleport points '{}' ({} total) from {}",
            self.config.name,
            self.point_count(),
            self.source_path
        );
        for (idx, point) in self.config.points.iter().enumerate() {
            let marker = if Some(idx) == self.active_index { "*" } else { " " };
            println!(
                "{} {:>3} | {:<28} | {} | ({:.6}, {:.6})",
                marker,
                idx + 1,
    ...
    fn print_status(&self) {
        if let Some(idx) = self.active_index {
            ...
            println!("🧭 No teleport target selected yet.");
        }
    }

    fn handle_shell_line(&mut self, line: &str) {
        let Some(command) = parse_teleport_shell_command(line) else {
            return;
        };
        let result = match command {
            TeleportShellCommand::Index(index) => self.queue_index(index),
            TeleportShellCommand::Next => self.queue_next(),
            TeleportShellCommand::Prev => self.queue_prev(),
            TeleportShellCommand::List => {
                self.print_list();
                Ok(())
            }
            TeleportShellCommand::Help => {
                self.print_help();
                Ok(())
            }
            TeleportShellCommand::Status => {
                self.print_status();
                Ok(())
            }
        };
...
fn parse_teleport_shell_command(line: &str) -> Option<TeleportShellCommand> {
    ...
    match parts.next() {
        Some("next") => Some(TeleportShellCommand::Next),
        Some("prev") => Some(TeleportShellCommand::Prev),
        Some("list") => Some(TeleportShellCommand::List),
        Some("help") => Some(TeleportShellCommand::Help),
        Some("status") => Some(TeleportShellCommand::Status),
        Some(index) => index.parse::<usize>().ok().map(TeleportShellCommand::Index),
        None => Some(TeleportShellCommand::Help),
    }
}

fn spawn_shell_command_reader() -> std::sync::mpsc::Receiver<String> {
    ...
    std::thread::spawn(move || {
        ...
        for line in stdin.lock().lines() {
            match line {
                ...
                Err(err) => {
                    eprintln!("🧭 Shell command reader stopped: {}", err);
                    break;
                }
            }
        }
...
fn compass_bearing_to_local_yaw_rad(
    local_pos: Vec3,
    compass_bearing_deg: f32,
    origin_gps: GPS,
    origin_voxel: VoxelCoord,
) -> f32 {
    let to_ne = |bearing_deg: f32| {
        let bearing_rad = bearing_deg.to_radians();
        Vec3::new(bearing_rad.cos(), bearing_rad.sin(), 0.0)
    };

    let Some(x_bearing) =
        open_world_forward_bearing_deg(local_pos, Vec3::X, origin_gps, origin_voxel)
    else {
        return compass_bearing_deg.to_radians();
    };
    let Some(z_bearing) =
        open_world_forward_bearing_deg(local_pos, Vec3::Z, origin_gps, origin_voxel)
    else {
        return compass_bearing_deg.to_radians();
    };

    let x_axis = to_ne(x_bearing);
    let z_axis = to_ne(z_bearing);
    let target = to_ne(compass_bearing_deg);
    let det = x_axis.x * z_axis.y - z_axis.x * x_axis.y;
    if det.abs() < 1e-6 {
        return compass_bearing_deg.to_radians();
    }

    let local_x = (target.x * z_axis.y - z_axis.x * target.y) / det;
    let local_z = (x_axis.x * target.y - target.x * x_axis.y) / det;
    if local_x.abs() < 1e-6 && local_z.abs() < 1e-6 {
        return compass_bearing_deg.to_radians();
    }

    local_z.atan2(local_x)
}
...
fn ensure_capture_target_chunks_loaded(
    ...
    let mut found_nonempty_level = false;
    for search_dy in 0..=48 {
        for signed_dy in if search_dy == 0 {
            vec![0]
        } else {
            vec![-search_dy, search_dy]
        } {
            let chunk_y = target_chunk_y + signed_dy;
            let mut found_here = false;
            for (dx, dz) in neighborhood {
                let chunk_id = ChunkId::new(target_chunk_x + dx, chunk_y, target_chunk_z + dz);
                if try_preload_capture_chunk(
                    chunk_streamer,
                    pending_mesh_queue,
                    tile_store,
                    chunk_id,
            ...
            if found_here {
                found_nonempty_level = true;
                break;
            }
        }
        if found_nonempty_level {
            break;
        }
    }
    ...
    for dy in -2..=1 {
        let chunk_id = ChunkId::new(target_chunk_x, target_chunk_y + dy, target_chunk_z);
        if !try_preload_capture_chunk(chunk_streamer, pending_mesh_queue, tile_store, chunk_id) {
...
fn find_anchor_voxel_near_target(
    chunk_streamer: &ChunkStreamer,
    target_lat: f64,
    target_lon: f64,
    prefer_water: bool,
    ...
    for candidate in columns.values() {
        ...
        let dist_m = worldgen_river::haversine_m(target_lat, target_lon, lat, lon);
...
fn capture_runner_pre_update(
    ...
    let Some(target_column_voxel) = runner.current_target_column_voxel(origin_gps) else {
    ...
    *player_mode = PlayerModeLocal::Fly;
    ...
    let staged_yaw_rad =
        compass_bearing_to_local_yaw_rad(staged_local, point.yaw_deg, origin_gps, origin_voxel);
...
fn teleport_controller_pre_update(
    controller: &mut TeleportController,
    ...
    if controller.stage != TeleportStage::TeleportPoint {
        return;
    }
    let Some(point) = controller.current_point().cloned() else {
        eprintln!("🧭 Teleport target disappeared before move");
        controller.stage = TeleportStage::Idle;
        return;
    };
    let Some(target_alt_m) = controller.current_target_alt_m(origin_gps) else {
        eprintln!("🧭 Teleport could not resolve target altitude");
        controller.stage = TeleportStage::Idle;
        return;
    };
    let Some(target_column_voxel) = controller.current_target_column_voxel(origin_gps) else {
        eprintln!("🧭 Teleport could not resolve target column");
        controller.stage = TeleportStage::Idle;
        return;
    };
    if *game_mode != GameMode::OpenWorld {
        println!("🧭 Leaving Construct — entering Open World for teleport");
    ...
    *player_chunk = ChunkId::from_ecef(&player.position);
    ...
    chunk_streamer.update(player.position);
    chunk_streamer.queue_priority(*player_chunk);
    controller.load_timed_out = false;
    controller.load_timeout_started_at = None;
    ensure_capture_target_chunks_loaded(
        chunk_streamer,
        pending_mesh_queue,
        tile_store,
    ...
    *loading_start = Instant::now();
    *loading_last_log = Instant::now();
    controller.stage = TeleportStage::WaitForLoading;
    println!(
        "🧭 Teleporting to tp {} / {}: {} ({})",
        controller.current_index_one_based().unwrap_or_default(),
...
fn capture_runner_post_update(
    ...
    match runner.stage {
        CaptureStage::WaitForLoading => {
            ...
            let snapped_yaw_rad = compass_bearing_to_local_yaw_rad(
                snapped_local,
                point.yaw_deg,
                origin_gps,
                origin_voxel,
...
fn teleport_controller_post_update(
    controller: &mut TeleportController,
    player: &mut Player,
    physics: &mut PhysicsWorld,
    chunk_streamer: &ChunkStreamer,
    ...
    if controller.stage != TeleportStage::WaitForLoading {
        return;
    }
    if game_loading {
        return;
    ...
    let generating = chunk_streamer.stats.chunks_loading;
    let queued = chunk_streamer.stats.chunks_queued;
    let standard_ready = loaded >= controller.config.min_loaded_chunks && generating == 0 && queued == 0;
    if !standard_ready && !controller.load_timed_out {
        return;
    }
    let Some(point) = controller.current_point().cloned() else {
        eprintln!("🧭 Teleport target disappeared after loading");
        controller.stage = TeleportStage::Idle;
        return;
    };
    let Some((anchor_voxel, anchor_distance_m)) = find_anchor_voxel_near_target(
        chunk_streamer,
        point.lat,
        point.lon,
        point.prefer_water,
        ...
        eprintln!(
            "🧭 Teleport failed — no terrain/water column found for tp {} ({})",
            point.id,
            point.display_label()
        );
        controller.stage = TeleportStage::Idle;
    ...
    if controller.load_timed_out {
        println!(
            "🧭 Teleport '{}' proceeding after load timeout with {} loaded, {} generating, {} queued",
            point.id, loaded, generating, queued
        );
    }
    let snapped_local = Vec3::new(
        (anchor_voxel.x - origin_voxel.x) as f32,
        (anchor_voxel.y - origin_voxel.y) as f32 + controller.current_ground_offset_m(),
        (anchor_voxel.z - origin_voxel.z) as f32,
    );
    if anchor_distance_m > 0.5 {
        println!(
            "🧭 Teleport point '{}' nudged {:.1}m to nearest populated column",
            point.id, anchor_distance_m,
        );
    }
    let snapped_yaw_rad = compass_bearing_to_local_yaw_rad(
        snapped_local,
    ...
    *player_chunk = ChunkId::from_ecef(&player.position);
    controller.stage = TeleportStage::Idle;
    println!(
        "🧭 Ready at tp {} / {}: {} ({}) | loaded={} generating={} queued={}",
        controller.current_index_one_based().unwrap_or_default(),
        controller.point_count(),
        point.id,
        point.display_label(),
...
fn capture_metadata_path(output_path: &Path) -> PathBuf {
    output_path.with_extension("json")
}

fn capture_diagnostic_image_path(output_path: &Path) -> PathBuf {
    let stem = output_path
        .file_stem()
        .and_then(|stem| stem.to_str())
        .unwrap_or("capture");
    output_path.with_file_name(format!("{}__diag.png", stem))
}

fn capture_view_label_for_layer(mode: LayerViewMode) -> &'static str {
    match mode {
        LayerViewMode::Off => "beauty",
        LayerViewMode::Height => "height",
        LayerViewMode::Slope => "slope",
        LayerViewMode::Ground => "ground",
        LayerViewMode::Hydro => "hydro",
        LayerViewMode::Roads => "roads",
        LayerViewMode::Buildings => "buildings",
    }
}
...
fn game_mode_label(mode: &GameMode) -> &'static str {
    match mode {
        ...
        GameMode::OpenWorld => "open_world",
    }
}

fn player_mode_label(mode: PlayerModeLocal) -> &'static str {
    match mode {
        PlayerModeLocal::Walk => "walk",
        PlayerModeLocal::Fly => "fly",
    }
}

fn material_label(mat: MaterialId) -> String {
    format!("{:?}", mat)
}
...
fn save_capture_record_json(record: &CaptureShotRecord, output_path: &Path) -> Result<(), String> {
    ensure_parent_dir(output_path)?;
    let json = serde_json::to_string_pretty(record)
        .map_err(|e| format!("failed to serialize capture record: {}", e))?;
    std::fs::write(output_path, json)
        .map_err(|e| format!("failed to write capture metadata {}: {}", output_path.display(), e))
}
            ...
            for col in 0..8u32 {
                ...
                for sy in 0..scale.max(1) {
                    for sx in 0..scale.max(1) {
                        let px = x + col * scale.max(1) + sx;
                        let py = y + row as u32 * scale.max(1) + sy;
...
fn draw_text_rgba(
    ...
    for ch in text.chars() {
        draw_char_rgba(img, ch, cx, y, scale, color);
        cx += (8 * scale) + scale;
    }
    cx
}

fn wrap_text_line(text: &str, max_chars: usize) -> Vec<String> {
...
fn diagnostic_lines_for_record(record: &CaptureShotRecord) -> Vec<String> {
    let mut lines = vec![
        format!(
            "kind={} point={} view={} layer={} obs={}",
            record.capture_kind,
            record.point_name,
            record.view,
            record.layer_view_mode,
            record.observability_mode,
        ),
        format!(
            "game={} player={} bearing_deg={}",
            record.game_mode,
            record.player_mode,
            record
                .camera_bearing_deg
        ...
        format!(
            "player_gps=({:.6}, {:.6}, {:.2}) alt_msl={:.2}",
            record.player_gps[0],
            record.player_gps[1],
            record.player_gps[2],
            record.player_orthometric_alt_m,
        ),
        format!(
            "player_local=({:.1}, {:.1}, {:.1}) chunk={}",
            record.player_local[0],
            record.player_local[1],
            record.player_local[2],
            record
                .player_chunk
        ...
        format!(
            "loaded={} queued={} loading={} loaded_this_frame={}",
            record
                .loaded_chunks
                .map(|v| v.to_string())
                .unwrap_or_else(|| "n/a".to_string()),
        ...
        format!(
            "chunk_state={} dist_m={} lod={} dirty={} modified={}",
            record
                .chunk_state
                .clone()
                .unwrap_or_else(|| "n/a".to_string()),
        ...
        format!(
            "surface_y={} ground_y={} top_y={} clearance={}",
            record
                .surface_y_f
                .map(|v| format!("{:.2}", v))
                .unwrap_or_else(|| "n/a".to_string()),
        ...
        format!(
            "materials current={} above={} below={} top={} ground={}",
            record.current_material,
            record.above_material,
            record.below_material,
            record
                .top_material
                .clone()
                .unwrap_or_else(|| "n/a".to_string()),
    ...
    if let Some(target_gps) = record.target_gps {
        lines.push(format!(
            "target_gps=({:.6}, {:.6})",
            target_gps[0], target_gps[1]
        ));
    }

    if record.active_chunk_id.is_some() {
        lines.push(format!(
            "active_chunk={} non_air={} water={} roads={} buildings={} ground_range={}..{}",
            record
                .active_chunk_id
                .map(|c| format!("[{},{},{}]", c[0], c[1], c[2]))
                .unwrap_or_else(|| "n/a".to_string()),
            record
                .active_chunk_non_air_columns
                .map(|v| v.to_string())
                .unwrap_or_else(|| "n/a".to_string()),
            record
                .active_chunk_water_columns
                .map(|v| v.to_string())
                .unwrap_or_else(|| "n/a".to_string()),
            record
                .active_chunk_road_columns
                .map(|v| v.to_string())
                .unwrap_or_else(|| "n/a".to_string()),
            record
                .active_chunk_building_columns
                .map(|v| v.to_string())
                .unwrap_or_else(|| "n/a".to_string()),
            record
                .active_chunk_min_ground_y
                .map(|v| v.to_string())
                .unwrap_or_else(|| "n/a".to_string()),
            record
                .active_chunk_max_ground_y
                .map(|v| v.to_string())
                .unwrap_or_else(|| "n/a".to_string()),
        ));
    }

    lines.push(format!("world_dir={}", record.world_dir));
    lines.push(format!("shared_data_root={}", record.shared_data_root));
    lines.push(format!("screenshot_root={}", record.screenshot_root));
    if let Some(region) = &record.region {
        lines.push(format!("region={}", region));
    }
    lines.push(format!("image={}", record.file));
    lines.push(format!("metadata={}", record.metadata_file));
    lines.push(format!("diagnostic={}", record.diagnostic_file));
    lines
}

fn capture_header_color(layer_view_mode: &str) -> image::Rgba<u8> {
    match layer_view_mode {
        "Hydro" => image::Rgba([45, 95, 180, 255]),
        "Height" => image::Rgba([100, 100, 110, 255]),
        "Slope" => image::Rgba([180, 110, 40, 255]),
        "Ground" => image::Rgba([80, 120, 65, 255]),
        "Roads" => image::Rgba([110, 110, 110, 255]),
        "Buildings" => image::Rgba([170, 90, 45, 255]),
        _ => image::Rgba([70, 70, 85, 255]),
    }
}

fn save_capture_diagnostic_card(
    source_image_path: &Path,
    diagnostic_path: &Path,
    record: &CaptureShotRecord,
) -> Result<(), String> {
    ensure_parent_dir(diagnostic_path)?;
    let preview = image::open(source_image_path)
        .map_err(|e| format!("failed to open capture image {}: {}", source_image_path.display(), e))?
        .resize(900, 520, image::imageops::FilterType::Triangle)
        .to_rgba8();

    let mut wrapped_lines = Vec::new();
    for line in diagnostic_lines_for_record(record) {
        wrapped_lines.extend(wrap_text_line(&line, 92));
    }

    let width = 1760u32;
    let header_h = 56u32;
    let margin = 24u32;
    let preview_panel_w = 940u32;
    let line_h = 14u32;
    let text_h = (wrapped_lines.len() as u32 * line_h) + 48;
    let height = (header_h + margin * 2 + preview.height().max(text_h) + 24).max(720);
    let mut canvas = image::RgbaImage::from_pixel(width, height, image::Rgba([12, 14, 18, 255]));
    fill_rect_rgba(
        &mut canvas,
        0,
        0,
        width,
        header_h,
        capture_header_color(&record.layer_view_mode),
    );
    fill_rect_rgba(
        &mut canvas,
        margin / 2,
        ...
        image::Rgba([20, 24, 30, 255]),
    );
    fill_rect_rgba(
        &mut canvas,
        preview_panel_w + margin,
        header_h + margin / 2,
        width - preview_panel_w - margin - margin / 2,
        height - header_h - margin,
        image::Rgba([18, 20, 26, 255]),
    );
    ...
    draw_text_rgba(
        ...
        16,
        2,
        image::Rgba([250, 250, 252, 255]),
    );
    draw_text_rgba(
        &mut canvas,
        &format!(
            "kind={} view={} layer={}",
            record.capture_kind, record.view, record.layer_view_mode
        ),
        420,
        20,
        1,
        image::Rgba([235, 235, 240, 255]),
    );
    draw_text_rgba(
        &mut canvas,
        "Rendered shot",
        ...
        header_h + 6,
        1,
        image::Rgba([190, 195, 205, 255]),
    );
    draw_text_rgba(
        &mut canvas,
        "Machine-readable state",
        preview_panel_w + margin + 12,
        header_h + 6,
        1,
        image::Rgba([190, 195, 205, 255]),
    );

    let mut y = header_h + margin + 20;
    ...
    canvas
        ...
        .map_err(|e| format!("failed to save diagnostic card {}: {}", diagnostic_path.display(), e))
}
...
fn build_capture_record(
    ...
    region: Option<&str>,
    game_mode: &GameMode,
    player_mode: PlayerModeLocal,
    observability_mode: ObservabilityMode,
    layer_view_mode: LayerViewMode,
    player_gps: GPS,
    player_orthometric_alt_m: f64,
    player_local: Vec3,
    camera_bearing_deg: f32,
    probe: Option<&ObservabilityProbe>,
    active_chunk_layer: Option<&ChunkLayerSummary>,
) -> CaptureShotRecord {
    let (capture_kind, point_name, target_gps, view_label) = match &request.kind {
        FrameCaptureKind::Manual => (
            "manual".to_string(),
            "manual".to_string(),
            None,
            capture_view_label_for_layer(layer_view_mode).to_string(),
        ),
        FrameCaptureKind::Route {
            point_name,
            target_gps,
            view,
        } => (
    ...
    let anchor_offset_m = target_gps.map(|target| {
        worldgen_river::haversine_m(target[0], target[1], player_gps.lat, player_gps.lon)
    });

    CaptureShotRecord {
        capture_kind,
        file: request.output_path.display().to_string(),
        diagnostic_file: diagnostic_path.display().to_string(),
        metadata_file: metadata_path.display().to_string(),
        point_name,
        view: view_label,
        target_gps,
        player_gps: [player_gps.lat, player_gps.lon, player_gps.alt],
        player_orthometric_alt_m,
        anchor_offset_m,
        player_local: [player_local.x, player_local.y, player_local.z],
        camera_bearing_deg: Some(camera_bearing_deg),
        player_chunk: probe.map(|p| [p.player_chunk.x, p.player_chunk.y, p.player_chunk.z]),
        ground_y: probe.and_then(|p| p.ground_y),
        top_y: probe.and_then(|p| p.top_y),
        surface_y_f: probe.and_then(|p| p.surface_y_f),
        top_material: probe.and_then(|p| p.top_material).map(material_label),
        ground_material: probe.and_then(|p| p.ground_material).map(material_label),
        current_material: probe
            .map(|p| material_label(p.current_material))
            .unwrap_or_else(|| material_label(MaterialId::AIR)),
        above_material: probe
            .map(|p| material_label(p.above_material))
            .unwrap_or_else(|| material_label(MaterialId::AIR)),
        below_material: probe
            .map(|p| material_label(p.below_material))
            .unwrap_or_else(|| material_label(MaterialId::AIR)),
        chunk_state: probe.and_then(|p| p.chunk_state).map(|state| format!("{:?}", state)),
        chunk_distance_m: probe.and_then(|p| p.chunk_distance_m),
        chunk_lod: probe.and_then(|p| p.chunk_lod),
        chunk_dirty: probe.and_then(|p| p.chunk_dirty),
        chunk_last_modified: probe.and_then(|p| p.chunk_last_modified),
        ...
        loaded_this_frame: probe.map(|p| p.loaded_this_frame),
        game_mode: game_mode_label(game_mode).to_string(),
        player_mode: player_mode_label(player_mode).to_string(),
        observability_mode: observability_mode.label().to_string(),
        layer_view_mode: layer_view_mode.label().to_string(),
        world_dir: world_dir.display().to_string(),
        shared_data_root: shared_data_root.display().to_string(),
        screenshot_root: screenshot_root.display().to_string(),
        region: region.map(|value| value.to_string()),
        active_chunk_id: active_chunk_layer.map(|chunk| [chunk.chunk_id.x, chunk.chunk_id.y, chunk.chunk_id.z]),
        active_chunk_last_modified: active_chunk_layer.map(|chunk| chunk.last_modified),
        active_chunk_non_air_columns: active_chunk_layer.map(|chunk| chunk.non_air_columns),
        active_chunk_water_columns: active_chunk_layer.map(|chunk| chunk.water_columns),
        active_chunk_road_columns: active_chunk_layer.map(|chunk| chunk.road_columns),
        active_chunk_building_columns: active_chunk_layer.map(|chunk| chunk.building_columns),
        active_chunk_min_ground_y: active_chunk_layer.and_then(|chunk| chunk.min_ground_y),
        active_chunk_max_ground_y: active_chunk_layer.and_then(|chunk| chunk.max_ground_y),
    }
}

fn save_capture_sidecars(
    request: &FrameCaptureRequest,
    world_dir: &Path,
    shared_data_root: &Path,
    screenshot_root: &Path,
    ...
    probe: Option<&ObservabilityProbe>,
    active_chunk_layer: Option<&ChunkLayerSummary>,
) -> Result<CaptureShotRecord, String> {
    let metadata_path = capture_metadata_path(&request.output_path);
    let diagnostic_path = capture_diagnostic_image_path(&request.output_path);
    ...
    save_capture_record_json(&record, &metadata_path)?;
    save_capture_diagnostic_card(&request.output_path, &diagnostic_path, &record)?;
    Ok(record)
}
...
fn shared_world_data_root(world_dir: &Path) -> PathBuf {
    if world_dir
        .file_name()
        .and_then(|name| name.to_str())
        == Some("world_data")
    {
        return world_dir.to_path_buf();
    }
    ...
    PathBuf::from("world_data")
}

fn screenshot_root_for_world_dir(world_dir: &Path) -> PathBuf {
    let shared_root = shared_world_data_root(world_dir);
    shared_root
        .parent()
        .map(|parent| parent.join("screenshot"))
        .unwrap_or_else(|| PathBuf::from("screenshot"))
}

fn runtime_state_root_for_world_dir(world_dir_override: Option<&str>) -> PathBuf {
    if let Some(world_dir) = world_dir_override {
        let shared_root = shared_world_data_root(Path::new(world_dir));
...
fn runtime_state_root_for_identity_or_world(
    identity_file_override: Option<&str>,
    world_dir_override: Option<&str>,
) -> PathBuf {
    if let Some(identity_file) = identity_file_override {
    ...
    runtime_state_root_for_world_dir(world_dir_override)
}

fn write_default_client_config_if_missing(path: &Path) {
    if path.exists() {
        return;
    }
    ...
    if let Ok(json) = serde_json::to_string_pretty(&NodeConfig::client_local_defaults()) {
        let _ = std::fs::write(path, json);
        eprintln!("📝 Created default client config at {}", path.display());
    }
}

fn load_client_config(args: &[String]) -> (NodeConfig, PathBuf, Option<String>) {
    let explicit_path = value_for_flag(args, "--config").map(PathBuf::from);
    let config_paths = if let Some(path) = explicit_path.clone() {
        vec![path]
    } else {
        vec![PathBuf::from("client.json"), PathBuf::from("node.json")]
    };

    for path in &config_paths {
        if path.exists() {
            if let Ok(text) = std::fs::read_to_string(path) {
                match serde_json::from_str::<NodeConfig>(&text) {
                    Ok(cfg) => return (cfg, path.clone(), None),
                    Err(e) => {
                        let msg = format!(
                            "❌ Client config parse error in {} — using local defaults: {}",
                            path.display(),
                            e
                        );
                        eprintln!("{}", msg);
                        return (NodeConfig::client_local_defaults(), path.clone(), Some(msg));
                    }
                }
            }
        }
    ...
    let default_path = explicit_path.unwrap_or_else(|| PathBuf::from("client.json"));
    write_default_client_config_if_missing(&default_path);
    (NodeConfig::client_local_defaults(), default_path, None)
}

fn apply_client_cli_overrides(config: &mut NodeConfig, args: &[String]) {
    if has_flag(args, "--temp-identity") {
        config.temp_identity = true;
    }
    if let Some(world_dir) = value_for_flag(args, "--world-dir") {
        config.world_dir = Some(world_dir);
    }
    if let Some(identity_file) = value_for_flag(args, "--identity") {
        config.identity_file = Some(identity_file);
    }
    if let Some(load_radius) = value_for_flag(args, "--load-radius-m")
        .and_then(|value| value.parse::<f64>().ok())
        .filter(|value| *value > 0.0)
    {
        config.chunk_load_radius_m = load_radius;
    }
    if let Some(unload_radius) = value_for_flag(args, "--unload-radius-m")
        .and_then(|value| value.parse::<f64>().ok())
        .filter(|value| *value > 0.0)
    {
        config.chunk_unload_radius_m = unload_radius;
    }
    if let Some(max_loaded) = value_for_flag(args, "--max-loaded-chunks")
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|value| *value > 0)
    {
        config.max_loaded_chunks = max_loaded;
    }
    if has_flag(args, "--network-on") {
        config.network_enabled = true;
    }
    if has_flag(args, "--network-off") {
        config.network_enabled = false;
    }
    if has_flag(args, "--bootstrap-on") {
        config.bootstrap_enabled = true;
    }
    if has_flag(args, "--bootstrap-off") {
        config.bootstrap_enabled = false;
    }
    if has_flag(args, "--security-on") {
        config.security_enabled = true;
    }
    if has_flag(args, "--security-off") {
...
pub fn main() {
    ...
    let (mut client_config, client_config_path, config_warning) = load_client_config(&cli_args);
    apply_client_cli_overrides(&mut client_config, &cli_args);
    if let Some(msg) = config_warning {
        eprintln!("{}", msg);
    }

    // ── Auto-update check (before window opens so exec-restart is clean) ──────
    if client_config.network_enabled {
    ...
    println!("⚙️  Client config: {}", client_config_path.display());
    println!();
    if client_config.network_enabled {
        println!("🌐 Networking: ON");
        println!("   - Auto-discovery via mDNS (localhost/LAN)");
        if client_config.bootstrap_enabled {
            println!("   - Remote bootstrap enabled");
        } else {
            println!("   - Remote bootstrap disabled (LAN/direct peers only)");
        }
        println!("   - Player state sync @ 20 Hz");
        println!("   - Voxel operations with CRDT");
        println!("   - World state persistence");
    } else {
        println!("📴 Networking: OFF");
        println!("   - Local/offline test mode");
        println!("   - No relay, bootstrap, or peer sync attempts");
    }
    if client_config.security_enabled {
        println!("🔐 Security checks: ON");
        println!("   - Key/signature/permission enforcement active");
    } else {
        println!("🧪 Security checks: OFF");
        println!("   - Local testing mode (key/security plumbing present but not enforced)");
    }
    println!();
    println!("Controls:");
    println!("  WASD - Move");
    ...
    println!("  Shell tp - With --teleport-points-file loaded, type `tp list` / `tp 1` in this terminal\n");

    println!("🔐 Initializing cryptographic identity...");

    // Detect first run before creating/loading the identity.
    // For local mode we skip signup friction unless security checks are enabled.
    let is_temp = client_config.temp_identity;
    ...
    let world_dir_override = client_config.world_dir.clone();
    // --region <name>  — override the active region (e.g. gympie, brisbane-cbd)
    let region_override = value_for_flag(&cli_args, "--region");
    let load_radius_override_m = Some(client_config.chunk_load_radius_m as f32);
    let unload_radius_override_m = Some(client_config.chunk_unload_radius_m as f32);
    let max_loaded_chunks_override = Some(client_config.max_loaded_chunks);
    ...
    let teleport_points_file = value_for_flag(&cli_args, "--teleport-points-file");
    let runtime_state_root = runtime_state_root_for_identity_or_world(
        client_config.identity_file.as_deref(),
        world_dir_override.as_deref(),
    );
    let shared_data_root = world_dir_override
        .as_deref()
        .map(|d| shared_world_data_root(Path::new(d)))
        .unwrap_or_else(|| runtime_state_root.join("world_data"));
    let identity_path = client_config
        .identity_file
        .as_ref()
        .map(PathBuf::from)
        .unwrap_or_else(|| runtime_state_root.join("identity.key"));
    // SAFETY: we set these process-level environment variables once during startup,
    // before the event loop or any background/network threads are created.
    unsafe {
        std::env::set_var("METAVERSE_IDENTITY_FILE", &identity_path);
        std::env::set_var(
            "METAVERSE_BOOTSTRAP_CACHE_FILE",
            runtime_state_root.join("bootstrap_cache.json"),
        );
        std::env::set_var("METAVERSE_DATA_DIR", &shared_data_root);
    }
    let needs_signup = client_config.security_enabled && !is_temp && !Identity::key_file_exists();
    ...
    if !needs_signup {
        println!("   Key: {}", identity_path.display());
    }

    println!("\n🌐 Starting multiplayer runtime...");

    // Clone identity for multiplayer (we need it later for player persistence)
    let mut multiplayer = MultiplayerSystem::new_with_runtime_config(
        identity.clone(),
        MultiplayerRuntimeConfig {
            network_enabled: client_config.network_enabled,
            bootstrap_enabled: client_config.bootstrap_enabled,
        },
    )
    .expect("Failed to create multiplayer system");
    multiplayer.perm_config = if client_config.security_enabled {
    ...
    if client_config.network_enabled {
        if client_config.peers.is_empty() {
            println!("📡 No explicit peers configured — waiting for LAN/bootstrap discovery");
        } else {
            for peer in &client_config.peers {
                println!("📡 Dialing configured peer: {}", peer);
                if let Err(e) = multiplayer.dial(peer) {
                    println!("⚠️  Failed to dial {}: {}", peer, e);
                }
            }
        }
        ...
        if client_config.bootstrap_enabled {
            ...
            println!("   Remote bootstrap disabled");
        }
    } else {
        println!("📴 Networking disabled by config — running local-only");
    }
    ...
    if client_config.network_enabled {
        ...
        println!("\n🏠 Local test mode active — no peer connectivity expected.\n");
    }
    println!();
    ...
    let mut app = GameApp {
        ...
        init: Some(Box::new(move |el: &ActiveEventLoop| -> GameHandlerFn {
            ...
            let shared_data_root = shared_world_data_root(&data_dir);
            let screenshot_root = screenshot_root_for_world_dir(&data_dir);
            // Elevation cache is global shared data — always under the default world_data dir,
            // not duplicated into an unrelated shell cwd when --world-dir points inside a
            // known world_data root.
            let elev_cache = shared_data_root.join("elevation_cache");
            let api_key = std::env::var("OPENTOPOGRAPHY_API_KEY").ok();
            let tile_fetcher = Arc::new(multiplayer.tile_fetcher());
            let permission_config = if client_config.security_enabled {
                PermissionConfig::default()
            } else {
                PermissionConfig::permissive()
            };
            ...
            let user_content = Arc::new(Mutex::new(UserContentLayer::with_config(
            ...
            // Advertise/cache announcements only when networking is enabled.
            if client_config.network_enabled {
            ...
            // will propagate the provider records once we connect to the relay.
            {
                ...
                if client_config.network_enabled && !startup_chunk_ids.is_empty() {
                    println!(
            ...
            let load_radius_m = load_radius_override_m.unwrap_or(150.0);
            let default_unload_radius_m = if load_radius_override_m.is_some() {
                load_radius_m + 100.0
            } else {
                250.0
            };
            let unload_radius_m = unload_radius_override_m
                .unwrap_or(default_unload_radius_m)
                .max(load_radius_m + 10.0);
            let max_loaded_chunks = max_loaded_chunks_override.unwrap_or_else(|| {
                if load_radius_override_m.is_some() || unload_radius_override_m.is_some() {
                    let default_stream = ChunkStreamerConfig::default();
                    let scale = (load_radius_m as f64 / default_stream.load_radius_m).max(0.35);
                    ((default_stream.max_loaded_chunks as f64) * scale * scale)
                        .ceil()
                        .max(600.0) as usize
                } else {
                    600
            ...
            println!(
                "   Chunk streaming radius: load {:.0}m | unload {:.0}m | max loaded {}",
                load_radius_m, unload_radius_m, max_loaded_chunks
            );
            let stream_config = ChunkStreamerConfig {
                load_radius_m: load_radius_m as f64,
                unload_radius_m: unload_radius_m as f64,
                max_loaded_chunks,
            ...
            if client_config.network_enabled && no_local_save {
            ...
            if client_config.network_enabled {
                ...
                println!("📰 Content fetch disabled in local mode");
            }
            ...
            let mut capture_runner = if let Some(route_file) = capture_route_file.as_deref() {
                ...
                let default_output_dir = screenshot_root.join(format!(
                    "{}-{}",
                    route_path
                        .file_stem()
                        .and_then(|stem| stem.to_str())
            ...
            let teleport_shell_rx = teleport_points_file
                .as_ref()
                .map(|_| spawn_shell_command_reader());
            let mut teleport_controller = if let Some(points_file) = teleport_points_file.as_deref()
            {
                let controller = TeleportController::from_file(Path::new(points_file))
                    .expect("failed to initialize teleport points");
                println!(
                    "🧭 Loaded teleport points '{}' from {}",
                    controller.config.name, controller.source_path,
                );
                println!(
                    "   {} points ready. Type `tp list` or `tp 1` in this terminal.",
                    controller.point_count()
                );
                                    ...
                                    match keycode {
                                        ...
                                        KeyCode::F12 => {
                                                ...
                                                .unwrap_or(false)
                                            {
                                                ...
                                                let output_path = screenshot_root
                                                    .join("manual")
                                                    .join(format!(
                                                        "manual-{}.png",
                                                        unix_timestamp_millis()
                        ...
                        WindowEvent::RedrawRequested => {
                            ...
                            let loading_timeout_secs = capture_runner
                                ...
                                .or_else(|| {
                                    teleport_controller
                                        ...
                                        .map(|controller| controller.config.loading_timeout_secs)
                                })
                                .unwrap_or(60);

                            if let Some(shell_rx) = teleport_shell_rx.as_ref() {
                                while let Ok(line) = shell_rx.try_recv() {
                                    if let Some(controller) = teleport_controller.as_mut() {
                                        controller.handle_shell_line(&line);
                                    }
                                }
                            }

                            if let Some(controller) = teleport_controller.as_mut() {
                                teleport_controller_pre_update(
                                    controller,
                                    &mut game_mode,
                                    &mut player,
                                    &mut player_mode,
                            ...
                            if game_loading {
                                ...
                                let capture_loading_ready = capture_runner
                                    ...
                                    .unwrap_or(false);
                                let teleport_loading_ready = teleport_controller
                                    .as_ref()
                                    .filter(|controller| {
                                        controller.stage == TeleportStage::WaitForLoading
                                    })
                                    .map(|controller| {
                                        loaded >= controller.config.min_loaded_chunks
                                            && generating == 0
                                            && queued == 0
                                            && pending_mesh_queue.is_empty()
                                            && controller
                                ...
                                if loading_last_log.elapsed().as_secs_f32() >= 1.0 {
                                    ...
                                    let player_status = if capture_loading_ready {
                                        "capture-ready"
                                    } else if teleport_loading_ready {
                                        "tp-ready"
                                ...
                                } else if loading_start.elapsed().as_secs() >= loading_timeout_secs
                                {
                                    ...
                                    if let Some(controller) =
                                        teleport_controller.as_mut().filter(|controller| {
                                            controller.stage == TeleportStage::WaitForLoading
                                        })
                                    {
                                        controller.load_timed_out = true;
                                        if controller.load_timeout_started_at.is_none() {
                                            controller.load_timeout_started_at = Some(Instant::now());
                                        }
                                    }
                                    println!(
                            ...
                            if let Some(controller) = teleport_controller.as_mut() {
                                teleport_controller_post_update(
                                    controller,
                                    &mut player,
                                    &mut physics,
                                    &chunk_streamer,
                            ...
                            match context.surface.get_current_texture() {
                                Ok(frame) => {
                                    ...
                                    let active_chunk_layer_any = if game_mode == GameMode::OpenWorld
                                    {
                                    ...
                                    let active_chunk_layer = if layer_view_mode != LayerViewMode::Off {
                                        active_chunk_layer_any
                                    } else {
                                        None
                                    };
                                    ...
                                    if let Some(request) = queued_frame_capture.take() {
                                        ...
                                        let log_label = request.log_label.clone();
                                        let is_route_capture = matches!(
                                            request.kind,
                                            FrameCaptureKind::Route { .. }
                                        );
                                                ...
                                                match save_capture_sidecars(
                                                    ...
                                                    Ok(record) => {
                                                        println!(
                                                            ...
                                                            record.diagnostic_file,
                                                        );
                                                        if is_route_capture {
                                                            if let Some(runner) = capture_runner.as_mut()
                                                            {
                                                                if let Err(err) = runner.record_capture(Some(record))
                                                                {
                                                                    runner.fail(err);
                                                                }
                                                            }
                                                    ...
                                                    Err(err) => {
                                                        eprintln!(
                                                            "📸 Screenshot diagnostics failed for {}: {}",
                                                            log_label, err
                                                        );
                                                        if is_route_capture {
                                                            if let Some(runner) = capture_runner.as_mut() {
