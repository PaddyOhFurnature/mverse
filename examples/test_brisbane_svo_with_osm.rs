//! Brisbane SVO with Real SRTM + OSM Data  
//! Tests full pipeline: SRTM → SVO → OSM CSG → Marching Cubes → Mesh
...
fn main() {
    ...
    // Load OSM data and apply CSG
    println!("\nLoading OSM data...");
    let osm_cache = DiskCache::new().expect("Failed to create cache");
    let osm_data: OsmData = match osm_cache.read_osm("brisbane_cbd") {
        Ok(bytes) => serde_json::from_slice(&bytes).expect("Failed to parse OSM"),
        Err(_) => {
            println!("✗ No OSM - continuing with terrain only");
            OsmData::default()
        }
    };
    
    if !osm_data.water.is_empty() {
        println!("✓ Loaded {} water features", osm_data.water.len());
        println!("\nCarving rivers...");
        let chunk_center = gps_to_ecef(&center);
        
        for (i, water) in osm_data.water.iter().enumerate().take(3) {
            if water.polygon.len() >= 2 {
                carve_river(&mut svo, &chunk_center, "river", &water.polygon, 30.0, voxel_size);
                println!("  Carved river {}/{}", i+1, 3.min(osm_data.water.len()));
            }
        }
    }
