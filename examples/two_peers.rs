...
use metaverse_core::network::{NetworkEvent, NetworkNode};
use std::env;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let identity = if peer_name == "peer1" {
        let path =
            std::path::PathBuf::from(shellexpand::tilde("~/.metaverse/peer1.key").to_string());
        if path.exists() {
        ...
        let path =
            std::path::PathBuf::from(shellexpand::tilde("~/.metaverse/peer2.key").to_string());
        if path.exists() {
            Identity::load_from_path(&path)?
        } else {
            let new_id = Identity::generate();
